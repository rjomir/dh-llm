...
const datasetOptions = [
  { label: 'COVID-QA', value: 'covid-qa' },
  { label: 'Drop', value: 'drop' },
  { label: 'Databricks Dolly', value: 'databricks-dolly' },
];
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await fetch(
        `http://localhost:5005/benchmark/${datasetSelected}`,
      );
        ...
        <Controller
          ...
          render={({ field, fieldState }) => (
            <>
              <label
                htmlFor="dataset"
                className={classNames({ 'p-error': errors.dataset })}
              >
                Select dataset:
              </label>
              <Dropdown
